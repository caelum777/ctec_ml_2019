...
# Curva ROC
prediccionesROC = ROCR::prediction(c(modelo.arbol.predicciones), c(accidentes.prueba$lesion))
...
text.adj = c(-0.2,1.7),
main = 'Curva ROC del modelo Arbol de Desicion')
```
...
# Curva ROC
prediccionesROC = ROCR::prediction(c(modelo.bosque.predicciones), c(accidentes.prueba$lesion))
as.numeric(performance(prediccionesROC, "auc")@y.values)
...
# Curva ROC
prediccionesROC = ROCR::prediction(c(modelo.regresion.predicciones), c(accidentes.prueba$lesion))
as.numeric(performance(prediccionesROC, "auc")@y.values)
...
# Curva ROC
prediccionesROC = ROCR::prediction(c(modelo.neural.predicciones$net.result), c(accidentes.prueba$lesion))
as.numeric(performance(prediccionesROC, "auc")@y.values)
...
print.cutoffs.at = seq(0,1,by = 0.1),
text.adj = c(-0.2,1.7),
main = 'Curva ROC del modelo Red Neural')
```
...
modelo.svm.performance <- performance(ROCR::prediction(c(modelo.svm.predict), c(accidentes.small.prueba$lesion)), measure = "auc")@y.values
print(modelo.neural.evaluacion)
print(modelo.svm.performance)

# Curva ROC
prediccionesROC = ROCR::prediction(c(modelo.svm.predict), c(accidentes.small.prueba$lesion))
as.numeric(performance(prediccionesROC, "auc")@y.values)
...
- El modelo de Regresion Logistica nos ha dado los mejores resultados, la curva ROC destaca de las demás y se nota que tiene mejor rendimiento.
- El Arbol de desicion y el Random Forest nos dan resultados muy similares, se puede notar al ver los graficos ROC de ambas son muy similares. Se podría mejorar el algoritmo de Random Forest aplicando un poco de tuning pero no se obtuvo resultados significativos.
- La red neuronal no es el mejor modelo para este caso, al tratar de agregar más inputs se hace inmanejable y dura mucho en correr el modelo.

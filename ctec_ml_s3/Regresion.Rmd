...
```{r}
hist(df_train$disp)
(disp_norm <- orderNorm(df_train$disp))
hist(disp_norm$x.t)

hist(df_train$weight)
(weight_norm <- orderNorm(df_train$weight))
hist(weight_norm$x.t)

hist(df_train$mpg)
(mpg_norm <- orderNorm(df_train$mpg))
hist(mpg_norm$x.t)
```

```{r}
reg <- lm(formula = mpg_norm$x.t ~ disp_norm$x.t + weight_norm$x.t, data = df_train)
reg
```

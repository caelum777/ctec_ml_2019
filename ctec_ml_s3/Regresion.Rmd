...
```{r}
hist(df_train$disp)
df_train$disp <- log10(df_train$disp)
hist(df_train$disp)

hist(df_train$weight)
df_train$weight <- log10(df_train$weight)
hist(df_train$weight)

hist(df_train$mpg)
df_train$mpg <- orderNorm(df_train$mpg)$x.t
hist(df_train$mpg)

df_test$disp <- log10(df_test$disp)
df_test$weight <- log10(df_test$weight)
df_test$mpg <- orderNorm(df_test$mpg)$x.t
```

```{r}
reg <- lm(formula = mpg ~ disp + weight, data = df_train)
#reg <- lessR::reg(mpg ~ disp + weight, data=df_train)
```
...
preds[1]
df_test$mpg[1]
mltools::mse(preds = preds, actuals=df_test$mpg)
